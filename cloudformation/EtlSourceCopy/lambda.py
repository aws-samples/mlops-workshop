        ...
        for stageState in response['stageStates']:
            if stageState['stageName'] == 'CopySourceCode':
                for actionState in stageState['actionStates']:
                    if actionState['actionName'] == 'ETLSource':
        ...
        for inputArtifacts in event["CodePipeline.job"]["data"]["inputArtifacts"]:
            ...
            if inputArtifacts['name'] == 'EtlSourceOutput':
                ...
                with zipfile.ZipFile(io.BytesIO(zip_bytes), "r") as z:
                   etl_script = z.read(file_name).decode('ascii')
                   s3_upload_path = executionId + "/input/etl/code/" + os.path.split(file_name)[-1]
                   s3.put_object(Bucket=output_bucket, Body=etl_script, Key=s3_upload_path)
